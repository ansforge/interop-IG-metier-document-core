...
* group[=].element[=].target.code = #Participant1.typeCode
* group[=].element[=].target.equivalence = #equivalent
...
// Sous-groupe de mapping : identité (référencée par associatedEntity) → CDA
// Participant1.associatedEntity référence le profil CDA SÉPARÉ fr-cda-associated-entity ;
// identifiant et nom y sont donc mappés dans un groupe dédié.
* group[+].source = "https://interop.esante.gouv.fr/ig/document-core/StructureDefinition/FRLMParticipant"
* group[=].target = "https://interop.esante.gouv.fr/ig/cda/document-core/StructureDefinition/fr-cda-associated-entity"
// Identifiant
* group[=].element[+].code = #FRLMParticipant.identifier
* group[=].element[=].target.code = #AssociatedEntity.id
* group[=].element[=].target.equivalence = #equivalent
// Nom
* group[=].element[+].code = #FRLMParticipant.name
* group[=].element[=].target.code = #AssociatedEntity.associatedPerson
* group[=].element[=].target.equivalence = #wider
* group[=].element[=].target.comment = "associatedPerson référence à son tour un profil CDA séparé (identité de la personne) ; le nom y est porté globalement."
